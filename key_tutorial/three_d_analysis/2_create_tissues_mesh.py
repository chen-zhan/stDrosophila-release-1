...
fb_mesh = sd.tl.build_three_d_model(
    adata=fb_adata, coordsby="spatial", groupby="anno",
    group_show=["fat body"], group_cmap=["#F56867"], group_amap=0,
    surf_color="#F56867", surf_alpha=0.5, cs_method="alpha_shape", cs_method_args={"alpha": 5}, smoothing=True, n_surf=10000,
    voxelize=True, voxel_size=[0.5, 0.5, 0.7], voxel_smooth=300,
)
sd.pl.easy_three_d_plot(mesh=fb_mesh, scalar="groups", ambient=0.3, opacity=0.5,
...
cns_mesh = sd.tl.build_three_d_model(
    adata=cns_adata, coordsby="spatial", groupby="anno",
    group_show=["CNS"], group_cmap=["#FEB915"], group_amap=0,
    surf_color="#FEB915", surf_alpha=0.5, cs_method="alpha_shape", cs_method_args={"alpha": 5}, smoothing=True, n_surf=10000,
    voxelize=True, voxel_size=[0.5, 0.5, 0.7], voxel_smooth=300,
)
sd.pl.easy_three_d_plot(mesh=cns_mesh, scalar="groups", ambient=0.3, opacity=0.5,
...
body_mesh = sd.tl.build_three_d_model(
    adata=adata, coordsby="spatial", groupby=None, group_amap=0,
    surf_color="gainsboro", surf_alpha=0.5, cs_method="basic", cs_method_args=None, smoothing=True, n_surf=10000,
    voxelize=True, voxel_size=[0.5, 0.5, 0.7], voxel_smooth=300)
...
p.show(screenshot=r"D:\BGIpy37_pytorch113\three_d_analysis_image\create_tissues_mesh\combination.png")