...
mesh1 = sd.tl.build_three_d_model(
    adata=adata,
    coordsby="spatial",
    groupby="anno",  # basic info
    group_show="all",
    group_cmap="rainbow",
    group_amap=0.5,  # group set
    gene_show="all",
    gene_cmap="hot_r",
    gene_amap=0.5,  # gene expression set
    mask_color="gainsboro",
    mask_alpha=0,  # mask set (not displayed)
    surf_color="gainsboro",
    surf_alpha=0.5,
    cs_method="basic",
    cs_method_args=None,
    smoothing=True,
    n_surf=10000,  # surface set
    voxelize=True,
    voxel_size=[0.5, 0.5, 0.7],
    voxel_smooth=300,  # voxelize set
    coodtype=np.float64,
    expdtype=np.float64,  # data type
)
...
mesh3 = sd.tl.build_three_d_model(
    adata=adata,
    coordsby="spatial",
    groupby=None,  # basic info
    group_show="all",
    group_cmap="rainbow",
    group_amap=0,  # group set
    gene_show="all",
    gene_cmap="hot_r",
    gene_amap=0.5,  # gene expression set
    mask_color="gainsboro",
