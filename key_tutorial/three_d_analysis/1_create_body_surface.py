...
pcd, surf_mesh = sd.tl.construct_three_d_mesh(
    adata=adata, coordsby="spatial", groupby="anno", key_added="groups",
    mask=None, mesh_style="surf", mesh_color="gainsboro", mesh_alpha=0.5,
    pcd_cmap="rainbow", pcd_amap=0.5, pcd_voxelize=False, pcd_voxel_size=[0.5, 0.5, 0.5],
    cs_method="basic", cs_method_args=None, surf_smoothness=500, n_surf=50000, vol_smoothness=200,
)

sd.pl.three_d_plot(
    mesh=surf_mesh, key="groups", off_screen=False, window_size=(1024, 768), background="black", background_r="white",
...
sd.tl.mesh_to_vtk(mesh=surf_mesh, filename=r"D:\BGIpy37_pytorch113\three_d_analysis_image\mesh_vtk\basic_surf.vtk")
