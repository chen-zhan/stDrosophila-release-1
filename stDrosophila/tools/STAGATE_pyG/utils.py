...
def Batch_Data(adata, batch_x, batch_y, spatial_key=['X', 'Y'], plot_Stats=False):
    ...
    batch_x_coor = [np.percentile(Sp_df[:, 0], (1/batch_x)*x*100) for x in range(batch_x+1)]
    batch_y_coor = [np.percentile(Sp_df[:, 1], (1/batch_y)*x*100) for x in range(batch_y+1)]
