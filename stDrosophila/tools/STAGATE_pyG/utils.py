...
def Cal_Spatial_Net(
    adata, rad_cutoff=None, k_cutoff=None, model="Radius", verbose=True
):
    """\
...
def Cal_Spatial_Net_3D(
    ...
    for temp_section in np.unique(adata.obs[key_section]):
        ...
        if verbose:
            print(
                "This graph contains %d edges, %d cells."
    ...
    for it in range(num_section - 1):
        ...
        temp_adata.uns["Spatial_Net"]["Section_id_1"] = temp_adata.uns["Spatial_Net"][
            "Cell1"
        ].map(spot_section_trans)
        temp_adata.uns["Spatial_Net"]["Section_id_2"] = temp_adata.uns["Spatial_Net"][
            "Cell2"
        ].map(spot_section_trans)
    ...
    if verbose:
        print(
            "3D SNN contains %d edges, %d cells."
...
def Batch_Data(adata, batch_x, batch_y, spatial_key=["X", "Y"], plot_Stats=False):
    Sp_df = adata.obs.loc[:, spatial_key].copy()
    Sp_df = np.array(Sp_df)
    batch_x_coor = [
        np.percentile(Sp_df[:, 0], (1 / batch_x) * x * 100) for x in range(batch_x + 1)
    ]
    batch_y_coor = [
        np.percentile(Sp_df[:, 1], (1 / batch_y) * x * 100) for x in range(batch_y + 1)
    ]

    Batch_list = []
    for it_x in range(batch_x):
        for it_y in range(batch_y):
    ...
    if plot_Stats:
        ...
        plot_df = pd.DataFrame(
            [x.shape[0] for x in Batch_list], columns=["#spot/batch"]
        )
