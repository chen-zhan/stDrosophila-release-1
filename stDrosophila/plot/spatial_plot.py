...
def spatial_plot(
    ...
    if isinstance(adata, list):
        ...
        adata_list = (
            [adata]
            if len(adata_ca) == 1
            else [adata[adata.obs[slice_col] == i] for i in adata_ca]
        )
    ...
    if cluster_col is None:
        ...
        cluster_ca.sort()
        cluster_color = {
            str(cluster): color for cluster, color in zip(cluster_ca, plot_color)
        }
        for num, sub_adata in enumerate(adata_list):
            ax = plt.subplot(ind_num, col_num, num + 1)
            ...
            palette = sns.color_palette(
                [cluster_color[str(i)] for i in sub_adata_cluster_ca]
            )
            sc.pl.spatial(
                sub_adata,
                spot_size=spot_size,
