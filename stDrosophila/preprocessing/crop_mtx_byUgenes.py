...
def _convert_binsize(bin1_data, binkt=None, binout=None):
    ...
    binkt_data['binkt_coords'] = binkt_data['x'].astype(str) + "_" + binkt_data['y'].astype(str)
    # binout
    ...
    binout_data = binout_data.reindex(columns=['geneID', 'x', 'y', 'MIDCounts', 'binkt_coords'])
    # merge
    binkt_data = binkt_data.groupby(['x', 'y', 'binkt_coords', 'geneID'])['MIDCounts'].sum().to_frame(
        'MIDCounts').reset_index()
    binkt_data = binkt_data.reindex(columns=['geneID', 'x', 'y', 'MIDCounts', 'binkt_coords'])
    binout_data = binout_data.groupby(['x', 'y', 'binkt_coords', 'geneID'])['MIDCounts'].sum().to_frame(
        'MIDCounts').reset_index()
    binout_data = binout_data.reindex(columns=['geneID', 'x', 'y', 'MIDCounts', 'binkt_coords'])
...
def filterUGenes(stage=None, ugfamily=None):
    avail_stages = {'stage 1-3 (BDGP in situ)', 'stage 4-6 (BDGP in situ)', 'stage 7-8 (BDGP in situ)',
    ...
    if ugfamily is None:
        ubiquitous_genes = stages_flydata[
...
def crop_midimg(data, bin=None, otsu=6, opening=2, contamination=0.05, med=3):
    ...
    def fill_gap(coords, bin):
        ...
        for name1, group1 in coords.groupby("x"):
            y_min, y_max = group1["y"].astype(float).min(), group1["y"].astype(float).max()
            fill_coords_list1.extend([[name1, i] for i in range(y_min, y_max + bin, bin)])
        ...
        for name2, group2 in fill_coords_data1.groupby("y"):
            x_min, x_max = group2["x"].astype(float).min(), group2["x"].astype(float).max()
            fill_coords_list2.extend([[i, name2] for i in range(x_min, x_max + bin, bin)])
    ...
    def edge_smoothing(coords, med):
        ...
        """
        coords['value'] = 1
        coords_table = pd.pivot_table(coords, index=["y"], columns=["x"], values="value", aggfunc=np.sum,
                                      fill_value=0).astype('float64')
        smooth_mtx = filters.median(coords_table.values, disk(med))
        smooth_table = pd.DataFrame(smooth_mtx, columns=coords_table.columns, index=coords_table.index)
    ...
    reslut_coords = [f"{int(smooth_coords['x'].loc[i])}_{int(smooth_coords['y'].loc[i])}" for i in smooth_coords.index]
    return reslut_coords


def cropbyUG(bin1_data, stage=None, ugfamily=None, binkt=5, binout=20, otsu=6, opening=2, contamination=0.05, med=3):
    """
    ...
    """
    binkt_data, binout_data = _convert_binsize(bin1_data=bin1_data, binkt=binkt, binout=binout)
    ...
    binkt_table = pd.pivot_table(binkt_data, index=["y"], columns=["x"], values="MIDCounts",
                                 aggfunc=np.sum, fill_value=0).astype('float64')
    new_coords = crop_midimg(data=binkt_table, bin=binkt, otsu=otsu, opening=opening,
                             contamination=contamination, med=med)

    binout_data = binout_data[binout_data['binkt_coords'].isin(new_coords)]
    binout_data.drop(columns='binkt_coord', axis=1)
    binout_data = binout_data.groupby(['x', 'y', 'geneID'])['MIDCounts'].sum().to_frame('MIDCounts').reset_index()
    return binout_data
